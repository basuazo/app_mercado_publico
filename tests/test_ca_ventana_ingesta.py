...
from app.clients.base import MPConcurrencyError, MPServerError, reset_rate_limiter_compartido
...
class _ApiFalsa:
    ...
    - `paginas`: páginas por ventana (1 por defecto); cada página trae un ítem
    ...
    def listar_compra_agil(self, **kw: Any) -> RespuestaListadoV2:
        ...
        total = self.total(*par) if self.total else len(items) * self.paginas
        return RespuestaListadoV2(
            items=items,
            paginacion=PaginacionV2(
                total_paginas=self.paginas,
...
class TestCortes:
    ...
    def test_corte_en_pagina_2_de_la_ventana_3(self, monkeypatch, session, engine, exc) -> None:
        settings = _settings(monkeypatch)
        _con_cursor(session, _AHORA - timedelta(hours=30))
    ...
    def test_reejecutar_tras_el_corte_no_duplica(self, monkeypatch, session, engine) -> None:
        settings = _settings(monkeypatch, ca_ventana_horas=6)
        ...
        with pytest.raises(MPServerError):
            sync_incremental(session, corte, settings)  # type: ignore[arg-type]
        ...
        assert cursor_tras_corte is not None and _cursor(engine) == _LIMITE
